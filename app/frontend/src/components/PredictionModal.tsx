    ...
    <Modal title="Predict availability" subtitle={station.name} onClose={onClose}>
      ...
      {prediction && prediction.status === 'prediction_available' && (
        <div className="prediction-result">
          <div
            className="probability-visual"
            style={{ '--probability': `${prediction.probability}%` } as React.CSSProperties}
          >
            <div>
          ...
          <div className="prediction-copy">
            <span className="confidence">
              <ShieldCheck size={16} /> {prediction.confidence} confidence
            </span>
            <h3>{prediction.message}</h3>
            <p>This is an estimate, not a guarantee. Live conditions may change before arrival.</p>
          </div>
          <div className="evidence-grid">
            <span>
              <Database size={17} />
              <b>{prediction.sampleSize}</b>
              <small>similar observations</small>
            </span>
            <span>
      ...
      {prediction && prediction.status !== 'prediction_available' && (
        <div className="empty-state compact">
          ...
          <p>
            {prediction.message} ({prediction.sampleSize} matching samples)
          </p>
        </div>
      )}
    </Modal>
