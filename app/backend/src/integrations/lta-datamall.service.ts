...
export class LtaDataMallService {
  ...
  constructor(private readonly config: ConfigService) {
    this.baseUrl =
      this.config.get('LTA_DATAMALL_BASE_URL') ?? 'https://datamall2.mytransport.sg/ltaodataservice';
  }
  ...
  async getAllStations(force = false): Promise<Station[]> {
    ...
    if (!force && this.cache && this.cache.expiresAt > Date.now())
      return structuredClone(this.cache.stations);
  ...
  normalizePayload(payload: unknown, fetchedAt = new Date().toISOString()): Station[] {
    ...
    return records
      .map((record, index) => this.normalizeStation(record, fetchedAt, index))
  ...
  private normalizeStation(record: JsonRecord, fetchedAt: string, index: number): Station | null {
    ...
    const connectorGroups = new Map<
      ConnectorType,
      { powers: number[]; statuses: unknown[]; prices: number[]; total: number; available: number }
    >();
    let operator = '';
    ...
    for (const point of chargingPoints) {
      ...
      for (const plug of plugTypes) {
        ...
        const power = this.number(
          plug.chargingSpeed ?? plug.ChargingSpeed ?? plug.powerRating ?? plug.PowerRating,
        );
        if (power !== null) group.powers.push(power);
        const priceType = this.string(plug.priceType ?? plug.PriceType)
          .toLowerCase()
          .replace('$', '');
        const price = this.number(plug.price ?? plug.Price);
        if (price !== null && price > 0 && (priceType.includes('kwh') || priceType === ''))
    ...
    const connectors: Connector[] = [...connectorGroups.entries()].map(([type, group]) => ({
      ...
      total: group.total,
      available: group.statuses.every((status) => status === undefined || status === null || status === '')
        ? null
        : group.available,
    ...
    const locationId =
      this.string(record.locationId ?? record.LocationId) || `${longitude.toFixed(6)}-${latitude.toFixed(6)}`;
    const address = this.string(record.address ?? record.Address);
    const name =
      this.string(record.name ?? record.Name) ||
      chargingPoints.map((point) => this.string(point.name ?? point.Name)).find(Boolean) ||
      address ||
      `EV charging station ${index + 1}`;
  ...
  private connectorType(value: string): ConnectorType | null {
    ...
    if (
      normalized.includes('ccs2') ||
      normalized === 'combo2' ||
      normalized.includes('combotype2') ||
      normalized.includes('ccscombo2')
    )
      return 'CCS2';
  ...
  private availabilityStatus(statuses: unknown[], available: number): AvailabilityStatus {
    ...
    if (
      ...
      statuses.every(
        (status) =>
          status === '' || status === null || status === undefined || this.statusCode(status) === 100,
      )
    )
      return 'offline';
    return 'unknown';
  ...
  private arrayOfRecords(value: unknown) {
    return Array.isArray(value) ? value.filter(this.record) : [];
  }
  ...
  private statusCode(value: unknown) {
    return value === '' || value === null || value === undefined ? null : this.number(value);
  }
  private postalCode(address: string, explicit: string) {
    return explicit || address.match(/\b\d{6}\b/)?.[0] || '';
  }
