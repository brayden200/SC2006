...
export class RecommendationsService {
  constructor(private readonly stationsService: StationsService, private readonly oneMap: OneMapService) {}
  ...
  async recommend(dto: RecommendationDto) {
    ...
    const ranked = (await Promise.all(search.stations.map(async (station, index) => {
      let route: RouteResult | null = null;
      if (index < 8) {
        try { route = await this.oneMap.drivingRoute(dto, station); } catch { route = null; }
      }
  ...
  rankStation(station: Station & { distanceKm?: number }, dto: RecommendationDto, route: RouteResult | null = null): RankedStation {
  ...
  async compare(dto: CompareStationsDto) {
    ...
    const known = (key: keyof (typeof options)[number]) => options.filter((item) => typeof item[key] === 'number');
